...
public:
  ...
  static auto QueryPostsDataProfileSinceLastByPageDesc(uint64_t lastId,
                                              int perPageNum = 10)
      -> std::vector<
          std::tuple<uint64_t, std::string, std::string, uint64_t, uint64_t,std::string,uint64_t>>;

  // Id, Title, Create Time, Update Time
static auto QueryPostsBasicStatusByPage(uint64_t lastId,int perPageNum = 10)      -> std::vector<
